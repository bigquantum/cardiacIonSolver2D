...
extern __constant__ bool circle_d;
...
__global__ void Cxy_field_kernel(stateVar conv, REAL3 c, REAL3 phi, bool *solid) {
  ...
  */
  ...
  if ( circle_d ) {
    ...
    conv.u[i2d] = solid[i2d] ? hy_d*y*c.t - c.x*cos(phi.t) + c.y*sin(phi.t) : 0.0;
    conv.v[i2d] = solid[i2d] ? -hx_d*x*c.t - c.x*sin(phi.t) - c.y*cos(phi.t) : 0.0;

  } else {

    conv.u[i2d] = hy_d*y*c.t - c.x*cos(phi.t) + c.y*sin(phi.t);
    conv.v[i2d] = -hx_d*x*c.t - c.x*sin(phi.t) - c.y*cos(phi.t);
...
void Cxy_field_wrapper(size_t pitch, dim3 grid2D, dim3 block2D, 
  stateVar conv, REAL3 c, REAL3 phi, bool *solid) {

  Cxy_field_kernel<<<grid2D, block2D>>>(conv, c, phi, solid);
  CudaCheckError();
...
switch (scheme) {
  ...
  case 1:
    ...
    if ( reduceSym ) {
      ...
      slice.ux[i2d] = convFB2ndOX(g.u,i,j,i2d,E,W,conv);
      slice.uy[i2d] = convFB2ndOY(g.u,i,j,i2d,N,S,conv);
      slice.vx[i2d] = convFB2ndOX(g.v,i,j,i2d,E,W,conv);
      slice.vy[i2d] = convFB2ndOY(g.v,i,j,i2d,N,S,conv);
  ...
  case 2:
    ...
    // #ifdef CIRCLE
      ...
      if ( reduceSym ) {
          ...
          slice.ux[i2d] = sc ? convFB2ndOX(g.u,i,j,i2d,E,W,conv) : 0.0;
          slice.uy[i2d] = sc ? convFB2ndOY(g.u,i,j,i2d,N,S,conv) : 0.0;
          slice.vx[i2d] = sc ? convFB2ndOX(g.v,i,j,i2d,E,W,conv) : 0.0;
          slice.vy[i2d] = sc ? convFB2ndOY(g.v,i,j,i2d,N,S,conv) : 0.0;
          // slice.ux[i2d] = MUSCLx(g.u,i,j,i2d,W,E);
          // slice.uy[i2d] = MUSCLy(g.u,i,j,i2d,S,N);
          // slice.vx[i2d] = MUSCLx(g.v,i,j,i2d,W,E);
          // slice.vy[i2d] = MUSCLy(g.v,i,j,i2d,S,N);
          ...
          if ( reduceSymStart ) {
            ...
            */
            ...
            slice0.ux[i2d] = convCentral2X(g.u,i,j,E,W);
            slice0.uy[i2d] = convCentral2Y(g.u,i,j,N,S);
            slice0.vx[i2d] = convCentral2X(g.v,i,j,E,W);
            slice0.vy[i2d] = convCentral2Y(g.v,i,j,N,S);
...
__device__ REAL convFB2ndOX(REAL *f, const int i, const int j, 
  ...
  return (conv.u[C]>0.0) ? (-3.0*f[C] + 4.0*f[E] - f[EE])*invdx_d :
...
__device__ REAL convFB2ndOY(REAL *f, const int i, const int j, 
  ...
  return (conv.v[C]>0.0) ? (-3.0*f[C] + 4.0*f[N] - f[NN])*invdy_d :
...
void slice_wrapper(size_t pitch, dim3 grid2D, dim3 block2D, 
  ...
  bool *intglArea, int *tip_count, vec5dyn *tip_vector) {
	...
	slice_kernel<<<grid2D, block2D>>>(g, slice, slice0,
		reduceSym, reduceSymStart, conv, scheme, intglArea,
    tip_count, tip_vector);
	CudaCheckError();
