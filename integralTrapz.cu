...
__global__ void trapz_kernel(REAL *f, REAL *g, REAL *h, REAL *w,
  ...
  //            h[index]*w[index] )*coeffTrapz[index]; /*gMask[index]**/
  ...
  int ic = i-nx_d/2;
  int jc = j-ny_d/2;
  int cx = __float2int_rn(tip_vector[*tip_count-1].x-nx_d/2);
  int cy = __float2int_rn(tip_vector[*tip_count-1].y-ny_d/2);

  // if (index == 0) printf("Iteration = %d\n", *tip_count);
  ...
  fxg = sc ? (f[index]*g[index] + h[index]*w[index]) /*gMask[index]**/ : 0.0 ; 
