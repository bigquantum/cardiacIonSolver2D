...
extern __constant__ bool circle_d, neumannBC_d;
  ...
  const int j = blockIdx.y*blockDim.y + threadIdx.y;
	...
	if ( neumannBC_d ) {
		...
		if ( circle_d ) {
			...
			g_out.u[i2d] = sc ? ue.u[i2d] - Rx*FDx - Ry*FDy : 0.0;
			...
			g_out.v[i2d] = sc ?  ue.v[i2d] - Rx*FDx - Ry*FDy : 0.0;
			...
			g_out.u[i2d] = ue.u[i2d] - Rx*FDx - Ry*FDy;
			...
			g_out.v[i2d] = ue.v[i2d] - Rx*FDx - Ry*FDy;
	    ...
	    if ( circle_d ) {
			...
			g_out.u[i2d] = sc ? uue - Rx*FDx - Ry*FDy : 0.0;
			...
			g_out.v[i2d] = sc ? vve - Rx*FDx - Ry*FDy : 0.0;
...
void advFDBFECC_wrapper(size_t pitch, dim3 grid2D, dim3 block2D,
	...
	advFDBFECC_kernel<<<grid2D,block2D>>>(pitch,gOut,gIn,conv,uf,ub,ue,solid);
	CudaCheckError();
