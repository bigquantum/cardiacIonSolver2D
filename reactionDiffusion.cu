...
__global__ void Adv_update_kernel(size_t pitch, stateVar g_out, stateVar g_in,
  stateVar conv, sliceVar slice) {
  ...
  */
  ...
  REAL du2dt = conv.u[i2d]*slice.ux[i2d] + conv.v[i2d]*slice.uy[i2d];
  REAL dv2dt = conv.u[i2d]*slice.vx[i2d] + conv.v[i2d]*slice.vy[i2d];
...
void Adv_update_wrapper(size_t pitch, dim3 grid2D, dim3 block2D,
   stateVar gOut_d, stateVar gIn_d, stateVar conv, sliceVar slice) {

  Adv_update_kernel<<<grid2D, block2D>>>(pitch, gOut_d, gIn_d, conv, slice);
  CudaCheckError();
