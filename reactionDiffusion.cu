...
__global__ void Adv_update_kernel(size_t pitch, stateVar g_out, stateVar g_in,
  ...
  */
  ...
  REAL du2dt = adv.x[i2d]*slice.ux[i2d] + adv.y[i2d]*slice.uy[i2d];
  REAL dv2dt = adv.x[i2d]*slice.vx[i2d] + adv.y[i2d]*slice.vy[i2d];
...
void Adv_update_wrapper(size_t pitch, dim3 grid2D, dim3 block2D,
  ...
  Adv_update_kernel<<<grid2D, block2D>>>(pitch, gOut_d, gIn_d, adv, slice);
  CudaCheckError();
