...
extern __constant__ bool circle_d, bilinear_d, tipGrad_d;
...
__global__ void spiralTip_kernel(size_t pitch, REAL *g_past,
  ...
  */
  ...
  REAL den1, den2, ctn1, ctn2, disc, px, py, sroot1, sroot2;
  float2 tip;
      ...
      if ( (ic*ic + jc*jc) < tipOffsetX_d*tipOffsetY_d ) {
        ...
        if ( bilinear_d ) {
          ...
          */
          ...
          tipRecordPlane(i,j,tip,disc,physicalTime,g_present,
      ...
      if ( (ic*ic + jc*jc) < tipOffsetX_d*tipOffsetY_d ) {
        ...
        */
        ...
        if ( equals( u1, Uth_d, 1e-15 ) && equals( u2, Uth_d, 1e-15 ) ) {
          ...
          // float2 g = gradient(i,j,s,t,g_present);
          float2 g = make_float2(0.0,0.0);
          vec5dyn data = { .x=(float)(i+s), .y=(float)(j+t), 
            .vx=(float)(g.x), .vy=(float)g.y, .t=(float)physicalTime };
